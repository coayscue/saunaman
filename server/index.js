...
async function sendPendingReviewEmails() {
  try {
    ...
    for (const reservation of reservations) {
      ...
      const eventEnd = new Date(reservation.event.date.getTime() + (reservation.event.duration || 90) * 60 * 1000);
      if (eventEnd < now) {
        await sendReviewRequest(reservation.user, reservation.event, reservation);
