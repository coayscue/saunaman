...
publicRouter.post('/', async (req, res) => {
  try {
    ...
    if (event.booked) return res.status(400).json({ error: 'Event is already booked' });

    const user = await User.findById(userId);
    if (!user) return res.status(404).json({ error: 'User not found' });

    // Check existing reservation
    const existing = await Reservation.findOne({ event: eventId, user: userId, cancelled: false });
    if (existing) return res.status(400).json({ error: 'Already reserved' });

    // Handle credit usage
    if (usedCredit) {
      if (user.credits <= 0) return res.status(400).json({ error: 'No credits available' });
      user.credits -= 1;
      await user.save();
    }
    ...
    // Mark public events as booked once someone signs up
    if (event.type === 'public') {
      event.booked = true;
      await event.save();
    }

    // For private events, check capacity
    if (event.type === 'private') {
      const reservationCount = await Reservation.countDocuments({ event: eventId, cancelled: false });
...
publicRouter.post('/:id/cancel', async (req, res) => {
  try {
    ...
    if (reservation.cancelled) return res.status(400).json({ error: 'Already cancelled' });
    ...
    if (event.type === 'private' && payment && payment.stripe_payment_id !== 'credit_used') {
